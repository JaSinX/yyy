...
from confetka import dp,bot
from NEmain import States,async_session,create_tables
...
async def start_command(message: types.Message):
    ...
    await message.answer(f"Добро пожаловать, дорогой пользователь {message.from_user.first_name}!\nЭто бот ChronoEvent,ты можешь отправить мне задачи, а я буду напоминать о них тебе.\n Нажми на 'Добавить задачу' ", reply_markup=keyboard)
...
#     await message.answer(f"Вы отправили стикер с идентификатором: {sticker_id}")
...
"Добавить задачу". Если нужна помощь, то введите "/help".""")
    ...
    sticker_id = "CAACAgIAAxkBAAJDFGXlSPcffwbpc5984ks1JIzckDWKAAJLKAACLRepSzwhUemj-49aNAQ"
...
async def handle_expired_task(session, task):
    try:
        if task.status == TaskStatus.ACTIVE:  # Проверяем статус задачи
            ...
            if task_time_utc <= current_time_utc:
                ...
                if task.status != TaskStatus.OVERDUE:
                    ...
                    await asyncio.sleep(30)  # Сон перед удалением сообщения выбора действия
...
async def send_reminder(session, task):
    try:
        ...
        if task_time_utc <= current_time_utc and task.status != TaskStatus.OVERDUE:
            # Создаем инлайн клавиатуру с тремя кнопками: Выполнено(завершено), Отложить и Пропустить
            inline_keyboard = types.InlineKeyboardMarkup()
            inline_keyboard.add(
...
async def help_command(message: types.Message):
    help_text = "Привет! Я бот ChronoEvent.\n\n" \
                "Я могу помочь в управлении вашими задачами и напоминать вам о них.\n\n" \
                "Вам всего лищь нужно вести команды:\n"\
                "/start - Начать взаимодействие с ботом\n" \
                "Статистика - чтобы получить данные о задаче\n"\
                "Отменить - Отменить текущий процесс\n" \
...
async def main():
    ...
    dp.register_message_handler(Text(equals="Статистика", ignore_case=True))
